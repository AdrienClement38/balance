...
> ⚠️ Ce fichier est le **miroir** de `CLAUDE.md` (chargé, lui, par Claude Code).
> Les deux doivent rester autonomes et dire la même chose : **toute modification ici
> est à reporter dans `CLAUDE.md`**.
...
intégré local** de l'outil : le volet de prévisualisation *in-app*. Ce volet reste **local
à la machine en cours** et ne peut jamais s'afficher ailleurs.

**Il est INTERDIT d'utiliser un navigateur externe / réel** — tout outil qui pilote un
**vrai Chrome, Firefox ou Edge** via une extension « navigateur connecté » (côté Claude
Code : `mcp__claude-in-chrome__*`, et l'ancienne graphie `mcp__Claude_in_Chrome__*`).
